...
document.getElementById("wallpaper").addEventListener("click", () => {
  const hexMap = Array.from(
    ...
    .join(",");
  console.log("%c%s", `background-color: ${paper}; color: ${ink}; font-size: 32px;`, hexMap);
...
function addPortrait([pixels, label], forceBackgroundUpdate = false) {
  ...
  img.title = label;
  const rows = pixels.split(",").map((n) => parseInt(n, 16).toString(2).padStart(16, "0"));
...
const portraits = [
  ...
  ["0,2c,7e,e7,1c3,389,714,f20,8c00,880,5150,410,b118,7882,f866,fc10", "2021/09/29"],
  ["1ff0,3ff8,3ff8,300c,2004,2004,ae75,6216,6086,a085,2184,2024,23c4,1008,810,7e0", "2021/09/17"],
  ["0,0,402,ca4,1820,3cc8,2c00,2010,20,40,0,0,30,40,e0,160", "2021/09/16"],
  ["ffff,c7e3,8181,0,701c,2830,3458,0,100,440,380,0,0,1c70,2288,100", "2021/09/14"],
  ["0,1010,e00f,0,0,200,600,0,0,2010,4008,4004,600c,1c78,380,0", "2021/09/13"],
  ["ffff,e003,c001,8000,8e1c,962c,8000,8080,8000,8220,81c0,8000,8410,c3e1,e003,ffff", "2021/09/10"],
  ["804,7f8,1ffc,3ffc,3ffc,300c,2004,2e74,2214,2104,2186,2425,d3c8,d008,2ff1,1002", "2021/09/04"],
